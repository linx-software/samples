
<div class="mydocy" style="display: flex; flex-direction: column;">
<h1>Linx Samples Library</h1> 

1. [System Integrations](#SystemIntegrations)
1. [Backend Automations](#BackendAutomations)
1. [API's and microservices](#APIs)
1. [Utilities](#Utilities)

<h2 id="SystemIntegrations">System Integrations</h2>

|Repo|Description|Linx Versions|
|:-----|:-----|:-----|
|[CloudConvert](https://github.com/linx-software/cloudconvert-integration)|Bulk convert files from and to a variety of formats|5|
|[Copper CRM](https://github.com/linx-software/copper)|Manage users, people and opportunities using <a href="https://developer.copper.com/" target="_blank">Copper CRM</a>|5|
|[Email to Slack](https://github.com/linx-software/email-to-slack)|Monitor GMail accounts and post incoming messages on Slack|5, 6|
|[GitHub Repos](https://github.com/linx-software/github)|Manage Github repositories|5, 6|
|[Microsoft Graph](https://github.com/linx-software/microsoft-graph-api-connectors)|Access data from <a href="https://learn.microsoft.com/en-us/graph/overview#whats-in-microsoft-graph" target="_blank">Microsoft Cloud Services</a>|5|
|[Optimo-Route](https://github.com/linx-software/optimo-route)|Create and retreive orders from <a href="https://optimoroute.com/" target="_blank">OptimoRoute</a> |5|
|[Salesforce Accounts](https://github.com/linx-software/salesforce-api-connectors)|Create and manage Salesforce accounts|5|
|[Salesforce Jobs](https://github.com/linx-software/salesforce-automation)|Create and manage Salesforce jobs|5|
|[Sharpspring](https://github.com/linx-software/sharpspring-api)|Get leads and events from SharpSpring|5, 6|
|[Shopify](https://github.com/linx-software/shopify-api-integration)|Receive event notifications from Shopify stores|5|

<h2 id="BackendAutomations">Backend Automations</h2>

|Repo|Description|Linx Versions|
|:-----|:-----|:-----|
|[Camunda Workflow](https://github.com/linx-software/Camunda-Workflow)|Use workflow engines to manage state|5|
|[GitHub Commits](https://github.com/linx-software/github-devops-management)|Retrieve Github commit activity and notify stakeholders via email|5, 6|
|[GitHub/Slack](https://github.com/linx-software/github-slack-devops-management)|Monitor Github repos and send Slack messages about commits and issues|5, 6|
|[GoogleDrive](https://github.com/linx-software/google-drive-file-management)|Manage and sync GoogleDrive files and meta data|5, 6|
|[Linux Servers](https://github.com/linx-software/linux-integration)|Use PuTTY to send commands to Linux machines|5|
|[Telegram Bot](https://github.com/linx-software/Telegram-Bot-Backend)|Automatically respond to Telegram messages|6|

<h2 id="APIs">API's and microservices</h2>

|Repo|Description|Linx Versions|
|:-----|:-----|:-----|
|[Linx-OAuth token service](https://github.com/linx-software/linx-oauth2-token-service)|Securely generate and store OAuth 2.0 access tokens|5|
|[RealWorld App](https://github.com/linx-software/realworld-api)|A Linx backend for the <a href="https://github.com/gothinkster/realworld" target="_blank">RealWorld</a> demo|5, 6|
|[REST Unixtime Converter](https://github.com/linx-software/unixtime)|Convert Unixtimestamps to DateTime objects and vice versa|5, 6|
|[SignalR chat](https://github.com/linx-software/signalr-chat)|Update web apps in real-time|5, 6|
|[Static site backend](https://github.com/linx-software/static-website-backend)|Build secure HTML web apps with JS and REST|5, 6|
|[Swagger Petstore](https://github.com/linx-software/petstore-api)|A Linx backend for the <a href="https://github.com/swagger-api/swagger-petstore" target="_blank">Petstore</a> demo|5, 6|

<h2 id="Utilities">Utilities</h2>

|Repo|Description|Linx Versions|
|:-----|:-----|:-----|
|[Generating PDF Reports](https://github.com/linx-software/razor-templates-pdf-report)|Personalise text using models and Razor templates|5|
|[Generating Random Strings](https://github.com/linx-software/generate-random-strings)|Generate random strings|6|
|[Populating docx](https://github.com/linx-software/docx-populate-placeholders)|Replace content in .docx files|6|

</div>
<div style="display: flex; flex-direction: column; align-items: center;"></div>

[Documentation](https://linx.software/docs/?utm=gh) | [Community](https://community.linx.software/community?utm=gh) | [Blog](https://linx.software/blog/?utm=gh) | [YouTube](https://www.youtube.com/channel/UCO4KWEv8nUzeaFRO4zKS4gA) | [Twitter](https://twitter.com/LinxCode)

[![](https://img.shields.io/badge/-Built_with_Linx-gray.svg?style=for-the-badge&labelColor=gray8&color=white&logo=data:image/svg%2bxml;base64,)](https://linx.software/)

</div>
